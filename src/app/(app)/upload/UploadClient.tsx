...
// locationText is optional at the Zod level; USER-mandatory enforcement
// happens at submit time (frontend) and in the API route (backend).
...
// isAdmin is resolved server-side and passed as a prop — no client session timing issues
export default function UploadClient({ isAdmin }: { isAdmin: boolean }) {
  ...
  const onSubmit = async (data: UploadForm) => {
    ...
    // Masyarakat must provide a location (isAdmin is server-resolved, always accurate)
    if (!isAdmin && !data.locationText) {
      setFieldError("locationText", { message: "Lokasi harus diisi" });
      return;
    }
    ...
    if (!postRes.ok) {
      ...
      // Show location error under the field, not the generic top bar
      if (json.error === "Lokasi harus diisi") {
        setFieldError("locationText", { message: "Lokasi harus diisi" });
      } else {
      ...
      {mode === "form" && (
        ...
        >
          ...
          <div className="space-y-1.5">
            ...
            <LocationPicker
              onSelect={(loc) => {
                setValue("locationText", loc.locationText, { shouldValidate: true });
